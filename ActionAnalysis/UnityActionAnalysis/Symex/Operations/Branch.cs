        ...
        public override void Perform(SymexState state)
        {
            ...
            if (satCases.Count > 1)
            {
                for (int i = 0, n = satCases.Count - 1; i < n; ++i)
                {
                    ...
                    if (SymexMachine.Instance.Config.ShouldAbortBranchCase(satCase.branchCase, Instruction, state))
                    {
                        fork.execStatus = ExecutionStatus.ABORTED;
                    } else
                    {
                        fork.EnqueueOp(new Fetch(satCase.branchCase.IP, Instruction));
                    }
                }
                SATCase lastSatCase = satCases[satCases.Count - 1];
                state.pathCondition.Add(lastSatCase.condition);
                if (SymexMachine.Instance.Config.ShouldAbortBranchCase(lastSatCase.branchCase, Instruction, state))
                {
            ...
            {
                SATCase satCase = satCases[0];
                state.pathCondition.Add(satCase.condition);
                if (SymexMachine.Instance.Config.ShouldAbortBranchCase(satCase.branchCase, Instruction, state))
                {
                    state.execStatus = ExecutionStatus.ABORTED;
                } else
                {
                    Fetch fetchOp = new Fetch(satCase.branchCase.IP, Instruction);
                    fetchOp.Perform(state);
                }
            } else
            {
