...
namespace UnityActionAnalysis
{
    public class Helpers
    {
        ...
        public static Expr MakeDefaultValue(IType type)
        {
            ...
            {
                throw new SymexUnsupportedException("MakeDefaultValue for type of kind " + type.Kind + " not implemented");
            }
        }
        ...
        public static bool GetSymcallIdFromVarName(string variableName, out int symcallId)
        {
            if (variableName.StartsWith("symcall:")) 
            {
                ...
                int idLength = accessorSepIndex < 0 ? variableName.Length-8 : accessorSepIndex-8;
        ...
        public static bool IsInputVariable(FuncDecl variable, SymexState s)
        {
        ...
        {
            if (e.IsConst && e.FuncDecl.DeclKind == Z3_decl_kind.Z3_OP_UNINTERPRETED && IsInputVariable(e.FuncDecl, s))
            {
                return true;
            }
            else
            ...
            while (changed)
            {
                ...
                foreach (BoolExpr cond in s.pathCondition)
                {
                    if (!result.Contains(cond))
                    {
                        ...
                        {
                            if (relevantVars.Contains(v.Name) || IsInputVariable(v, s))
                            {
                                include = true;
                                break;
                            }
