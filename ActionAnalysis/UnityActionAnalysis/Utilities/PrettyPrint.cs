...
namespace UnityActionAnalysis
{
    public class PrettyPrint
    {
        ...
        private string ExprSimplifyToString(Expr e)
        {
            ...
            if (funcName == "if")
            {
                if (e.Arg(2).FuncDecl.Name.ToString() == "bv2int")
                {
                    return ExprSimplifyToString(e.Arg(2).Arg(0));
                } else
                {
        ...
        public string ExprToPrettyString(Expr e, SymexState st)
        {
            if (e is IntExpr && e.FuncDecl.DeclKind == Z3_decl_kind.Z3_OP_ANUM)
            {
            ...
            } else
            {
                ...
                foreach (FuncDecl v in vars)
                {
                    ...
                    if (name.StartsWith("symcall:"))
                    {
                        ...
                        int accessorSepIndex = name.IndexOf(':', prefixLen);
                        int symcallId;
                        string accessor;
                        if (accessorSepIndex >= 0)
                        {
                            symcallId = int.Parse(name.Substring(prefixLen, accessorSepIndex - prefixLen));
                            accessor = name.Substring(accessorSepIndex + 1);
                        }
                        else
                        {
                            symcallId = int.Parse(name.Substring(prefixLen));
                            accessor = null;
                        }
