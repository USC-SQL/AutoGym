...
namespace UnityActionAnalysis
{
    public class InputInstrumentation
    {
        ...
        public static void Run(GameConfiguration config)
        {
            ...
            var decompiler = ActionAnalysis.LoadAssembly(config, config.assemblyFileName);
            string instrInputTypeName = "UnityActionAnalysis.InstrInput";
            ModuleDefinition module = ActionAnalysis.LoadAssemblyCecil(config, config.assemblyFileName);
            ...
            ActionAnalysis.PrepareAssembly(module, decompiler, config);

            module.Write(config.assemblyFileName + ".tmp");
            module.Dispose();
            var origFileName = config.assemblyFileName + ".orig";
            if (File.Exists(origFileName))
            {
                File.Delete(origFileName);
            }
            File.Move(config.assemblyFileName, origFileName);
