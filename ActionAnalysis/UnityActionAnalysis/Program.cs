    ...
    public class Program
    {
        ...
        public class Options
        {
            ...
            [Option("instrument",
                Required = false,
                Default = false,
                HelpText = "Instead of analyzing the assembly, instrument the assembly's invocations to Input APIs to be compatible with InputSimulator")]
        ...
        private static GameConfiguration ParseConfiguration(JsonElement configJson)
        {
            ...
            ISet<string> configIgnoreClasses;
            if (configJson.TryGetProperty("ignoreClasses", out JsonElement ignoreClasses))
            {
                if (ignoreClasses.ValueKind != JsonValueKind.Array)
                {
                    throw new Exception("config ignoreClasses should be an array");
                }
                int numElems = ignoreClasses.GetArrayLength();
                configIgnoreClasses = new HashSet<string>();
                for (int i = 0; i < numElems; ++i)
                {
                    JsonElement elem = ignoreClasses[i];
                    configIgnoreClasses.Add(elem.GetString());
                }
            } else
            {
                configIgnoreClasses = new HashSet<string>();
            ...
            bool configSlicingOpt = true;
            if (configJson.TryGetProperty("slicingOpt", out JsonElement slicingOpt))
            {
                configSlicingOpt = slicingOpt.GetBoolean();
            }
        ...
        {
            if (args[0] == "--altcover-fixup")
            {
                Parser.Default.ParseArguments<AltCoverInstrFixup.Program.Options>(args).WithParsed(AltCoverInstrFixup.Program.Run);
            } else
            {
