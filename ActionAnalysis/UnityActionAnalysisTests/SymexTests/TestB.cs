...
namespace UnityActionAnalysis.Tests
{
    ...
    public class TestB
    {
        ...
        {
            using (SymexMachine machine = SymexTestHelpers.CreateMachine("UnityActionAnalysisTestCases.Symex.TestB.ProgramB", "Main", new TestConfig()))
            {
                machine.Run();
                ...
                using (var z3 = new Context(new Dictionary<string, string>() { { "model", "true" } }))
                {
                    ...
                    {
                        if (SymexTestHelpers.ModelContainsVariables(m, arg0, arg1, arg2))
                        {
