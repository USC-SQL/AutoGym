...
namespace UnityActionAnalysis.Tests
{
    public class ConfigJ : TestConfig
    {
        ...
        public override void ApplyMethodSummary(IMethod method, List<Expr> arguments, Variable resultVar, SymexState state)
        {
            ...
            Expr result = MakeSymcall(method, arguments, symId, state);
    ...
    public class TestJ
    {
        ...
        {
            using (SymexMachine machine = SymexTestHelpers.CreateMachine("UnityActionAnalysisTestCases.Symex.TestJ.ProgramJ", "Main", new ConfigJ()))
            {
                machine.Run();
                ...
                using (var z3 = new Context(new Dictionary<string, string>() { { "model", "true" } }))
                {
                    ...
                    {
                        if (SymexTestHelpers.ModelContainsVariables(m, symcall0))
                        {
                    ...
                    foreach (var c in cases2)
                    {
                        Assert.IsTrue(helper.ExistsState((s, m) =>
                        {
                            if (SymexTestHelpers.ModelContainsVariables(m, symcall0, symcall1))
                            {
