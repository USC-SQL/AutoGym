...
namespace UnityActionAnalysis.Tests
{
    ...
    public class TestE
    {
        ...
        {
            using (SymexMachine machine = SymexTestHelpers.CreateMachine("UnityActionAnalysisTestCases.Symex.TestE.ProgramE", "Main", new TestConfig()))
            {
                machine.Run();
                ...
                using (var z3 = new Context(new Dictionary<string, string>() { { "model", "true" } }))
                {
                    ...
                    {
                        if (SymexTestHelpers.ModelContainsVariables(m, arg1, recordId))
                        {
