...
namespace UnityActionAnalysis.Tests
{
    public class ConfigI : TestConfig
    {
        ...
        {
            return base.IsMethodSummarized(method) || method.FullName == "UnityActionAnalysisTestCases.Symex.TestI.Record..ctor";
        }
    }
    ...
    public class TestI
    {
        ...
        {
            using (SymexMachine machine = SymexTestHelpers.CreateMachine("UnityActionAnalysisTestCases.Symex.TestI.ProgramI", "Main", new ConfigI()))
            {
                machine.Run();
                ...
                using (var z3 = new Context(new Dictionary<string, string>() { { "model", "true" } }))
                {
                    ...
                    foreach (var c in cases1)
                    {
                        ...
                        {
                            if (SymexTestHelpers.ModelContainsVariables(m, r1x))
                            {
                    ...
                    foreach (var c in cases2)
                    {
                        Assert.IsTrue(helper.ExistsState((s, m) =>
                        {
                            if (SymexTestHelpers.ModelContainsVariables(m, r1x, r2x))
                            {
                                int x1 = (int)uint.Parse(m.Evaluate(r1x).ToString());
