...
namespace UnityActionAnalysis.Tests
{
    ...
    public class TestD
    {
        ...
        {
            using (SymexMachine machine = SymexTestHelpers.CreateMachine("UnityActionAnalysisTestCases.Symex.TestD.ProgramD", "Main", new TestConfig()))
            {  
                machine.Run();
                ...
                using (var z3 = new Context(new Dictionary<string, string>() { { "model", "true" } }))
                {
                    ...
                    {
                        Console.WriteLine(m);
                        if (SymexTestHelpers.ModelContainsVariables(m, arg1, arg3, p1FavoriteColor, p2FavoriteColor))
                        {
                    ...
                    Assert.IsTrue(helper.ExistsState((s, m) =>
                    {
                        if (SymexTestHelpers.ModelContainsVariables(m, arg1, arg3))
                        {
                            int id1 = (int)uint.Parse(m.Evaluate(arg1).ToString());
                            int id2 = (int)uint.Parse(m.Evaluate(arg3).ToString());
