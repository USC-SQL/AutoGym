...
namespace UnityActionAnalysis.Tests
{
    ...
    public class TestH
    {
        ...
        {
            using (SymexMachine machine = SymexTestHelpers.CreateMachine("UnityActionAnalysisTestCases.Symex.TestH.ProgramH", "Main", new TestConfig()))
            {
                machine.Run();
                ...
                using (var z3 = new Context(new Dictionary<string, string>() { { "model", "true" } }))
                {
                    ...
                    foreach (var c in cases)
                    {
                        ...
                        {
                            if (SymexTestHelpers.ModelContainsVariables(m, runMode, x, y, z))
                            {
                    ...
                    Assert.IsTrue(helper.ExistsState((s, m) =>
                    {
                        if (SymexTestHelpers.ModelContainsVariables(m, runMode))
                        {
                            var rm = ulong.Parse(m.Evaluate(runMode).ToString());
                            return rm < 1 || rm > 10;
                        } else
